...
export default function DownloadsTable({
    ...
    return (
        <div className={`w-full font-sans text-slate-800 ${className || ''}`}>
            <div className={`grid grid-cols-1 ${enableFiltering ? 'lg:grid-cols-[260px_1fr]' : 'grid-cols-1'} gap-12`}>
                {/* Sidebar Filters */}
                {enableFiltering && (
                ...
                <div className="flex flex-col min-h-[600px]">
                                ...
                                <TableRow className="border-none hover:bg-transparent">
                                    ...
                                    {showCategoryColumn && (
                                        <TableHead>
                                            {enableSorting ? (
                                                <button
                                                    onClick={() => handleSort('primaryDownloadCategory')}
                                                    className="font-bold text-slate-900 text-xs uppercase tracking-wider flex items-center gap-1 hover:text-slate-600"
                                                >
                                                    {categoryHeader}
                                                    {sortConfig?.field === 'primaryDownloadCategory' && (
                                                        sortConfig.direction === 'asc' ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />
                                                    )}
                                                </button>
                                            ) : (
                                    ...
                                    <TableHead className="text-right pr-0">
                                        <span className="font-bold text-slate-900 text-xs uppercase tracking-wider">{downloadHeader}</span>
                                    </TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                    ...
                    {/* Shadcn Pagination */}
                    {enablePagination && items.length > 0 && totalPages > 1 && (
